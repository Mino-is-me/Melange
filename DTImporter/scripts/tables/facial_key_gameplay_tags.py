...
def import_table(sheet_id=SHEET_ID, gid=GID):
    try:
        ...
        print(f"표정 키 게임플레이 태그 임포트 중 오류 발생: {str(e)}")
        import traceback
        traceback.print_exc()
        return {'success': False, 'error': str(e)}
...
def process_and_save_data(data_table, data):
    try:
        ...
        temp_csv_path = os.path.join(temp_dir, "temp_facial_key_gameplay_tags.csv")
        ...
        with open(temp_csv_path, 'w', encoding='utf-8', newline='') as outfile:
            ...
            for row in data:
                ...
                facial_key = str(row.get('FacialKey', '')).strip()
                dev_comment = str(row.get('DevComment', '')).strip()

                if not row_name or not facial_key:
                    continue
